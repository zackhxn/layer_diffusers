...
class PNDMScheduler(SchedulerMixin, ConfigMixin):
    ...
    def step(
        ...
        """
        # print("PNDMschedular,self.counter",self.counter)
        if self.counter < len(self.prk_timesteps) and not self.config.skip_prk_steps:
            # print("PNDMschedular,self.prk",self.counter)
            return self.step_prk(model_output=model_output, timestep=timestep, sample=sample, return_dict=return_dict)
        else:
            # print("PNDMschedular,self.step_plms",self.counter)
    ...
    def step_plms(
        ...
        # print("PNDMscheduler,prev_timestep = timestep - self.config.num_train_timesteps // self.num_inference_steps",prev_timestep,timestep,self.config.num_train_timesteps,self.num_inference_steps)
        # print("PNDMscheduler",self.counter)
        # print("PNDMscheduler, self.ets",type( self.ets), len(self.ets))
        # print("PNDMscheduler, model_output",type(model_output), model_output.shape) #1,4,64,64
        if self.counter != 1:
            self.ets = self.ets[-3:]
            self.ets.append(model_output)
        else:
